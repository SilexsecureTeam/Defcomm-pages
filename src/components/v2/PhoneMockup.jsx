...
export default function PhoneMockup({
  ...
  const steps = tabs.map((tab, index) => ({
    ...
    disableBeacon: index === 0, // Start with a beacon on the first step
    placement: "right", // Position tooltip to the right of the sidebar
  }));

  // Handle Joyride callback to manage tour state
  ...
  return (
    <>
      <Joyride
        ...
        styles={{
          options: {
            primaryColor: "#89AF20", // Match the app's green accent
          ...
          {activeTab && (
            ...
            >
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  id={`tab-${tab.id}`} // Unique ID for Joyride targeting
                  onClick={() => setActiveTab(tab.id)}
